...
object GptParser {
     ...
     * Usually 33 sectors (33 * 512 = 16,896 bytes) or at least 2 sectors.
     */
    fun parseRawGpt(gptBytes: ByteArray): List<PartitionEntry> {
        if (gptBytes.size < 1024) {
            return emptyList()
        }

        // LBA 1: GPT Header (offset 0 if passed starting from LBA1, or 512 if starting from MBR LBA0)
        var headerOffset = 0
        if (!isGptHeader(gptBytes, 0)) {
            if (gptBytes.size >= 1024 && isGptHeader(gptBytes, 512)) {
                headerOffset = 512
            } else {
                return emptyList()
            }
        ...
        // Partition entries start at LBA 2 (usually offset 512 after header, or headerOffset + 512)
        var entriesOffset = headerOffset + 512
        ...
        for (i in 0 until numEntries) {
            ...
            // Calculate Addresses & Sizes (assuming standard 512-byte sector size)
            val sectorSize = 512L
